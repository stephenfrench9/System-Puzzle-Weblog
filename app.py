...
def index():
    ...
    sql_location = """SELECT COUNT(*) FROM weblogs WHERE source LIKE \'0\';"""
    cur.execute(sql_location)
    local = cur.fetchone()[0]

    # find all the remote requests
    sql_location = """SELECT COUNT(*) FROM weblogs WHERE source LIKE \'1\';"""
    cur.execute(sql_location)
    remote = cur.fetchone()[0]

    # find all the exotic requests
    sql_location = """SELECT COUNT(*) FROM weblogs WHERE source LIKE \'2\';"""
    cur.execute(sql_location)
    exotic = cur.fetchone()[0]

    # Get the number of successful local attempts
    sql_fail = """SELECT COUNT(*) FROM weblogs WHERE status LIKE \'2__\' AND source LIKE \'0\';"""
    cur.execute(sql_fail)
    success_local = cur.fetchone()[0]

    # Get the number of failed local attempts
    sql_fail = """SELECT COUNT(*) FROM weblogs WHERE status NOT LIKE \'2__\' AND source LIKE \'0\';"""
    cur.execute(sql_fail)
    fail_local = cur.fetchone()[0]

    # Get the number of successful remote attempts
    sql_fail = """SELECT COUNT(*) FROM weblogs WHERE status LIKE \'2__\' AND source LIKE \'1\';"""
    cur.execute(sql_fail)
    success_remote = cur.fetchone()[0]

    # Get the number of failed remote attempts
    sql_fail = """SELECT COUNT(*) FROM weblogs WHERE status NOT LIKE \'2__\' AND source LIKE \'1\';"""
    cur.execute(sql_fail)
    ...
    return render_template('index.html', rate=rate, all=all, success=success, failure=fail,
                           total=total, local=local, remote=remote, exotic=exotic, total2=total2,
                           success_local=success_local, fail_local=fail_local,
                           success_remote=success_remote, fail_remote=fail_remote,
                           local_sum=local_sum, remote_sum=remote_sum,
                           local_rate=local_rate, remote_rate=remote_rate)
